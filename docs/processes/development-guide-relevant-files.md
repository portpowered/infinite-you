...
| `tests/functional/default/` | Migrated fast functional collections | The default functional lane now has explicit package ownership under this tree, with lane execution through `make test-functional-default`. |
...
- During the transition out of `tests/functional_test`, keep any temporary fixture-resolution shim narrow and point it only at explicit collection-owned `tests/functional/.../testdata` roots rather than reviving a new mixed shared fixture bucket.
...
- When migrating CLI/docs/init smoke coverage into `tests/functional/default/...`, keep small working-directory and fixture helpers package-local unless multiple collections need the same seam; do not promote one-package helpers into `tests/functional/support/`.
