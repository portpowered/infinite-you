...
2. Read the PRD at `prd.json` and the progress log at `progress.txt` (check Codebase Patterns first).
3. Read and follow relevant standards from docs/standards/.
4. Implement the changes, keeping them focused and minimal.
5. Run quality checks (typecheck, lint, test) and fix any failures.
6. Commit with message: `feat: [Story ID] - [Story Title]`
7. Use `git add <specific files>` — never `git add .` or `git add -A`.
